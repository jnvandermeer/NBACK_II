...
# write a nice little python function that *SENDS* code stuff
# take the python function(s) that i wrote up for the NF as an example
# this should send to all (UDP-like) clients that might be listening a code
# port (not) needed??
...
def send_the_code(code):
    ...
    if SEND_UDP:
        # TRY IT!
        this_socket.sendto(f'{code}'.encode(), ('30.100.0.1',6500))
