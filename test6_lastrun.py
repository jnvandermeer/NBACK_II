...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\PsychoPy\\Tasks\\Nback\\test6_lastrun.py',
...
# keep track of which components have finished
eoComponents = [text_2, key_resp, p_port]
...
while continueRoutine:
    ...
    if p_port.status == NOT_STARTED and t >= 0.0-frameTolerance:
        ...
        win.callOnFlip(p_port.setData, int(10))
    if p_port.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > p_port.tStartRefresh + 180-frameTolerance:
            # keep track of stop time/frame for later
...
for thisWakeup in wakeup:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if p_port_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > p_port_3.tStartRefresh + 0.20-frameTolerance:
                # keep track of stop time/frame for later
    ...
    if p_port_3.status == STARTED:
        win.callOnFlip(p_port_3.setData, int(0))
    wakeup.addData('p_port_3.started', p_port_3.tStart)
    wakeup.addData('p_port_3.stopped', p_port_3.tStop)
    win.color=[0, 0, 0]
    ...
    if skip_this_routine:
        continueRoutine=False
    # keep track of which components have finished
    bComponents = [img_black, p_port_4]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if p_port_4.status == NOT_STARTED and t >= 0.0-frameTolerance:
            ...
            win.callOnFlip(p_port_4.setData, int(2))
        if p_port_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > p_port_4.tStartRefresh + 0.20-frameTolerance:
                # keep track of stop time/frame for later
    ...
    if p_port_4.status == STARTED:
        win.callOnFlip(p_port_4.setData, int(0))
    wakeup.addData('p_port_4.started', p_port_4.tStart)
    wakeup.addData('p_port_4.stopped', p_port_4.tStop)
    win.color=[0, 0, 0]
    thisExp.nextEntry()
