...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\PsychoPy\\Tasks\\NBACK_II\\test9_lastrun.py',
...
for thisEoecloop in eoecloop:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    ociClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = ociClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=ociClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    for thisComponent in ociComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    eoecloop.addData('text_2.started', text_2.tStartRefresh)
    eoecloop.addData('text_2.stopped', text_2.tStopRefresh)
    # check responses
    ...
    eoecloop.addData('key_resp.started', key_resp.tStartRefresh)
    eoecloop.addData('key_resp.stopped', key_resp.tStopRefresh)
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    ocClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    for thisComponent in ocComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    eoecloop.addData('text_8.started', text_8.tStartRefresh)
    eoecloop.addData('text_8.stopped', text_8.tStopRefresh)
    # check responses
